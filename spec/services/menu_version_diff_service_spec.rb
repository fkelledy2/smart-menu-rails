...
RSpec.describe MenuVersionDiffService, type: :service do
  it 'detects added/removed items and changed fields' do
    ...
    changed_item_ids = diff.dig(:items, :changed).pluck(:id)
    removed_item_ids = diff.dig(:items, :removed).pluck(:id)
    added_item_names = diff.dig(:items, :added).pluck(:name)
    ...
    changed_fields = changes_for_a.pluck(:field)
