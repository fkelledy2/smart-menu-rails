...
class MockGoogleVisionService
  def self.detect_labels(_image_path)
    [
  ...
  def self.detect_text(_image_path)
    "Sample Menu\nPizza Margherita $12.99\nCaesar Salad $8.50\nGrilled Chicken $15.99"
  end

  def self.detect_web_entities(_image_path)
    {
      web_entities: [
  ...
  def self.detect_objects(_image_path)
    [
      { name: 'Pizza', score: 0.95, bounding_box: { x: 100, y: 100, width: 200, height: 150 } },
    ]
  end

  def self.detect_landmarks(_image_path)
    []
  end
end
