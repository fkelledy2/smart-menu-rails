...
## Status
- Priority Rank: #23 (Phase 2 agent — requires in-app rating/feedback system to be active; meaningful only after restaurants have been live long enough to receive reviews)
- Category: Post-Launch — agent tier, Phase 2
- Effort: M
- Dependencies: Agent Framework (#17), in-app rating at checkout (confirm existing), `Ordr` / `Ordritem` models (existing), email mailers (existing), OpenAI API
