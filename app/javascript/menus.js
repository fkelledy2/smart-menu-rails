...
export function initMenus() {
  $(window).on('activate.bs.scrollspy', function () {
    ...
    $.data(
      ...
      'scrollTimer',
      setTimeout(function () {
        const element = document.querySelector('.menu_sections_tab a.active');
        if (element) {
  ...
  if ($('#restaurantTabs').length) {
    ...
    const restaurantMenuTable = new Tabulator('#restaurant-menu-table', {
      dataLoader: false,
      maxHeight: '100%',
      responsiveLayout: true,
      layout: 'fitDataStretch',
    ...
    restaurantMenuTable.on('rowMoved', function (row) {
      ...
      for (let i = 0; i < rows.length; i++) {
        restaurantMenuTable.updateData([
