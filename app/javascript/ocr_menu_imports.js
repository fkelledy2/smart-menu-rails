...
export function initOCRMenuImportDnD() {
  ...
  function initConfirmationToggles() {
    ...
    const getSectionCheckboxes = () =>
      Array.from(sectionsContainer.querySelectorAll('input.form-check-input[data-section-id]'));
  ...
  (function initInlineSectionDescriptionEdit() {
    ...
    sections.forEach((section) => {
      ...
      if (!descEl.querySelector('.edit-indicator')) {
        ...
        try {
          if (window.bootstrap?.Tooltip) new bootstrap.Tooltip(icon);
        } catch (_) {}
      }
  ...
  function persist(url, payload, label) {
    ...
    fetch(url, {
      ...
      .then((r) =>
        r.text().then((t) => {
          console.log(`[OCR DnD] Persist ${label} status`, r.status, t);
        })
      )
  ...
  function initSections() {
    ...
    function currentSectionIds() {
      return Array.from(container.querySelectorAll('.section-container')).map((el) =>
        parseInt(el.getAttribute('data-menu-section-section-id'), 10)
      );
    }
    ...
    function initFallback() {
      ...
      container.addEventListener('dragover', (e) => {
        ...
        if (closest.el) container.insertBefore(dragging, closest.el);
        ...
        new Sortable(container, {
          ...
          onChoose: (evt) => {
            console.log(
              ...
              evt.oldIndex,
              'id=',
              evt.item?.dataset?.menuSectionSectionId
            );
          },
          onStart: (evt) => {
            console.log(
              '[OCR DnD][sections] onStart index=',
              evt.oldIndex,
              'id=',
              evt.item?.dataset?.menuSectionSectionId
            );
  ...
  // Items per section
  function initItems() {
    const itemReorderUrl =
      container.getAttribute('data-reorder-items-url') ||
      (function () {
    ...
    document.querySelectorAll('[id^="section-"][id$="-items"]').forEach((el) => {
      ...
      function persistItems() {
        ...
        const url =
          itemReorderUrl ||
          (document.querySelector('meta[name="ocr-items-reorder-url"]') || {}).content;
        if (!url) {
          return;
        }
        ...
        try {
          ...
          new Sortable(itemContainer, {
            ...
            setData: (dataTransfer, dragEl) => {
              try {
                dataTransfer.setData('text', dragEl.dataset.itemId || '');
              } catch (_) {}
            },
            onFilter: (evt) => {},
            onChoose: (evt) => {},
      ...
      itemContainer.addEventListener('dragover', (e) => {
        ...
        if (closest.el) itemContainer.insertBefore(dragging, closest.el);
        else itemContainer.appendChild(dragging);
      });
    });
  }
  ...
  (function initInlineImportTitleEdit() {
    ...
    const onClickTitle = (e) => {
      ...
      const host =
        e.currentTarget?.closest?.('.import-title-editable') || e.currentTarget || titleEl;

      // Avoid multiple editors
      if (host.querySelector('input[data-role="import-title-input"]')) return;

      const originalText = (host.childNodes[0]?.textContent || host.textContent || '')
        .replace(/\s*$/, '')
        .trim();
      ...
      const persist = () => {
        ...
        if (!newName) {
          showInlineError('Please enter a name.');
          input.focus();
          return;
        }
