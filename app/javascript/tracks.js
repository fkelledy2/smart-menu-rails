export function initTracks() {
  ...
  if ($('#restaurantTabs').length) {
    ...
    function link(cell, formatterParams) {
      ...
      const rowData = cell.getRow().getData('data').externalid;
    ...
    const restaurantTracksTable = new Tabulator('#restaurant-tracks-table', {
      height: '400px',
      dataLoader: false,
    ...
    window.onSpotifyWebPlaybackSDKReady = () => {
      ...
      player.addListener(
        ...
        ({ position, duration, track_window: { current_track } }) => {
          const row = restaurantTracksTable.searchData('name', '=', current_track.name);
          if (row != null) {
    ...
    restaurantTracksTable.on('rowMoved', function (row) {
      ...
      for (let i = 0; i < rows.length; i++) {
        restaurantTracksTable.updateData([
