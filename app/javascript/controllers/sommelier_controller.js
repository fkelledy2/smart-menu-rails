...
export default class extends Controller {
  ...
  _filterWineColorButtons(wineColors) {
    ...
    const wineColorStep = this.stepTargets.find(
      (el) => el.dataset.flow === 'wine' && el.querySelector("[data-action*='setWineColor']")
    );
  ...
  _advanceFlow() {
    ...
    if (step) {
      const visibleButtons = Array.from(step.querySelectorAll('.sommelier-option-btn')).filter(
        (btn) => !btn.classList.contains('d-none')
      );
  ...
  renderResults(recommendations, isWine = false) {
    ...
    const cards = recommendations
      .map((rec, i) => (isWine ? this.buildWineCard(rec, i) : this.buildCard(rec, i)))
      .join('');
  ...
  buildWineCard(rec, index) {
    ...
    const wineMeta = [];
    if (rec.wine_color)
      wineMeta.push(rec.wine_color.charAt(0).toUpperCase() + rec.wine_color.slice(1));
    ...
    const wineMetaHtml =
      wineMeta.length > 0 ? `<div class="sommelier-wine-meta">${wineMeta.join(' · ')}</div>` : '';
  ...
  renderPairingsModal(data) {
    ...
    const similarHtml = (data.similar || [])
      .filter((s) => s.on_menu)
      .map(
        (s) => `
    ...
    modal
      .querySelector('#sommelier-pairings-close-btn')
      .addEventListener('click', () => this.closePairingsModal());
    modal.addEventListener('click', (e) => {
      if (e.target === modal) this.closePairingsModal();
    });
  }
