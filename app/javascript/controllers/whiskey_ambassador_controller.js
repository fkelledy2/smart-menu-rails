...
export default class extends Controller {
  ...
  async showFlights() {
    ...
    try {
      const response = await fetch(this.flightsUrlValue, {
  ...
  _buildWhiskeyCard(rec, index) {
    ...
    const tags = (rec.tags || [])
      .map((t) => `<span class="wa-tag">${t.replace(/_/g, ' ')}</span>`)
      .join('');
  ...
  _renderExplore(data) {
    ...
    const quadrants = Object.entries(data.quadrants || {})
      .map(
        ([key, q]) => `
