...
export default class extends Controller {
  ...
  async loadParticipantsAndPlan() {
    ...
    try {
      ...
      const orderResponse = await fetch(
        `/restaurants/${this.restaurantIdValue}/ordrs/${this.orderIdValue}.json`
      );
      if (!orderResponse.ok) {
      ...
      this.participants = (orderData.ordrparticipants || []).filter(
        (p) => p.role === 'customer' && p.sessionid
      );
      ...
      const planResponse = await fetch(
        `/restaurants/${this.restaurantIdValue}/ordrs/${this.orderIdValue}/split_plan`
      );
      if (planResponse.ok) {
  ...
  validateSplitPlan() {
    ...
    if (this.currentMethod === 'custom') {
      ...
      if (total !== this.orderTotal) {
        ...
        errors.push(
          `Total amounts must equal order total (off by ${this.formatCurrency(Math.abs(total - this.orderTotal))})`
        );
      }
  ...
  showExistingPlan() {
    ...
    const myShare = this.existingPlan.shares.find(
      (s) => s.ordrparticipant_id.toString() === this.participantIdValue
    );
  ...
  getSelectedParticipants() {
    const checkboxes = this.participantsListTarget.querySelectorAll(
