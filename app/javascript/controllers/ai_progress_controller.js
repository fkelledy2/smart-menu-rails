...
export default class extends Controller {
  ...
  _showConfirmModal() {
    ...
    // Set title per mode
    const titles = {
      localize: 'Translate Menu',
      polish: 'AI Polish Menu',
    ...
    if (this.hasPreviewTarget) {
      this.previewTarget.style.display =
        this.currentMode === 'localize' || this.currentMode === 'polish' ? 'none' : '';
    }

    // Clear log
    if (this.hasLogItemsTarget) this.logItemsTarget.innerHTML = '';
    if (this.hasLogWrapTarget) {
      this.logWrapTarget.classList.toggle(
        'd-none',
        !(this.currentMode === 'localize' || this.currentMode === 'polish')
      );
    }
  ...
  async _startJob() {
    ...
    const queuedTexts = {
      localize: 'Queued localization…',
      polish: 'Queued polishing…',
      ai: 'Queued…',
    };
    ...
    try {
      ...
      if (this.currentMode === 'localize') {
        const selected =
          this.element.querySelector('input[name="localizeMode"]:checked')?.value || 'missing';
      ...
      if (data && data.job_id) {
        ...
        const msg =
          resp.status >= 400 ? `Failed to start job (HTTP ${resp.status})` : 'Failed to start job';
  ...
  _pollProgress(form, jobId) {
    ...
    this.pollTimer = setInterval(async () => {
      try {
        ...
        const percent =
          total > 0 ? Math.floor((current / total) * 100) : status === 'completed' ? 100 : 0;
        ...
        if (
          this.currentMode === 'ai' &&
          p.current_item_image_url &&
          this.hasPreviewTarget &&
          this.hasPreviewImgTarget
        ) {
    ...
    document.addEventListener(
      ...
      function (e) {
        ...
        try {
          ...
          console.warn('Auto-save submit failed', err);
        }
      },
      true
    );
