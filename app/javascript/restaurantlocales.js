...
export function initRestaurantlocales() {
  ...
  if ($('#restaurantTabs').length) {
    ...
    function link(cell, formatterParams) {
      ...
      const rowData = cell.getRow().getData('data').locale;
    ...
    const restaurantLocaleTable = new Tabulator('#restaurant-locale-table', {
      dataLoader: false,
      maxHeight: '100%',
      responsiveLayout: true,
      layout: 'fitDataStretch',
      ajaxURL: '/restaurants/' + restaurantId + '/restaurantlocales.json',
      initialSort: [{ column: 'locale', dir: 'asc' }],
      columns: [
        {
    ...
    restaurantLocaleTable.on('rowMoved', function (row) {
      ...
      for (let i = 0; i < rows.length; i++) {
        restaurantLocaleTable.updateData([
