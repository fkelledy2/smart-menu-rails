...
export function initRestaurantavailabilities() {
  ...
  if ($('#restaurantTabs').length) {
    ...
    function link(cell, formatterParams) {
      ...
      return (
        "<a class='link-dark' href='/restaurantavailabilities/" + id + "/edit'>" + rowData + '</a>'
      );
    }
    ...
    const restaurantOpeningHourTable = new Tabulator('#restaurant-openinghour-table', {
      dataLoader: false,
      maxHeight: '100%',
      paginationSize: 20,
    ...
    restaurantOpeningHourTable.on('rowMoved', function (row) {
      ...
      for (let i = 0; i < rows.length; i++) {
        restaurantOpeningHourTable.updateData([
