...
export function initiInventories() {
  ...
  if ($('#menusection-inventory-table').length) {
    ...
    const restaurantId = $('#menusection-inventory-table').data('bs-restaurant');
    ...
    function link(cell, formatterParams) {
      ...
      const rowData = cell.getRow().getData('data').menuitem.name;
      return "<a class='link-dark' href='/inventories/" + id + "/edit'>" + rowData + '</a>';
    }
    const inventoryTable = new Tabulator('#menusection-inventory-table', {
      dataLoader: false,
      maxHeight: '100%',
      ...
      columns: [
        ...
        },
        {
          ...
          hozAlign: 'right',
          headerHozAlign: 'right',
          mutator: (value, data) => data.currentinventory + '/' + data.startinginventory,
        },
        {
          title: 'Resets At',
          field: 'resethour',
    ...
    document.getElementById('reset-inventory').addEventListener('click', function () {
      const rows = inventoryTable.getSelectedData();
      for (let i = 0; i < rows.length; i++) {
        inventoryTable.updateData([
          { id: rows[i].id, currentinventory: rows[i].startinginventory },
        ]);
        const r = {
          inventory: {
