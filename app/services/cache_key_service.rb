...
class CacheKeyService
  class << self
    ...
    def fetch_multiple(keys_and_options = {}, &block)
      if keys_and_options.empty?
        return {}
      end
      
      # Use Rails cache fetch_multi for efficient batch operations
      Rails.cache.fetch_multi(*keys_and_options.keys, &block)
    end
    ...
    def warm_menu_cache(menu)
      ...
      menu.menusections.includes(:menuitems).each do |section|
