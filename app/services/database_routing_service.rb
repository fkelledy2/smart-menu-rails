class DatabaseRoutingService
  class << self
    ...
    def with_analytics_connection(&block)
      ...
      ApplicationRecord.on_primary(&block)
    end
    
    # Execute read-heavy queries on replica
    def with_read_connection(&block)
      if replica_healthy?
        ApplicationRecord.on_replica(&block)
      else
    ...
    def route_query(query_type: :read, consistency: :eventual, &block)
      case query_type
      when :analytics, :reporting
        with_analytics_connection(&block)
      when :read
        case consistency
        when :strong, :immediate
          with_primary_connection(&block)
        when :eventual, :weak
