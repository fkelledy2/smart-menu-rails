...
class ImportToMenu
  ...
  def upsert_into_menu(menu, sync: false)
    ...
    stats = { sections_created: 0, sections_updated: 0, items_created: 0, items_updated: 0, sections_archived: 0, items_archived: 0 }
    ActiveRecord::Base.transaction do
      ...
      confirmed_sections.each do |section|
        ...
        Array(section.ocr_menu_items).select { |i| i.is_confirmed }.sort_by(&:sequence).each do |item|
  ...
  def validate_import!(allow_existing_menu: false)
    ...
    raise StandardError, 'No confirmed sections to import' unless @import.ocr_menu_sections.respond_to?(:confirmed) && @import.ocr_menu_sections.confirmed.any?
  end
  ...
  def normalize_allergens(arr)
    Array(arr).map { |x| x.to_s.strip }.reject(&:blank?).join(', ')
  end
  ...
  def sync_allergyns!(restaurant, allergen_names, archive_absent: false)
    # Normalize
    desired = allergen_names.map { |n| n.to_s.strip.downcase }.reject(&:blank?).uniq
  ...
  def map_item_allergyns!(menuitem, allergen_names, restaurant)
    desired = Array(allergen_names).map { |n| n.to_s.strip.downcase }.reject(&:blank?).uniq
    ...
    desired_ids = allergyns.map(&:id).to_set
