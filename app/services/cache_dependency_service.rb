...
class CacheDependencyService
  ...
  class << self
    ...
    def batch_invalidate(cache_keys, strategy: :cascade)
      ...
      grouped_keys.sort.reverse.each do |level, keys|
    ...
    def cascade_invalidate(cache_key)
      ...
      Rails.logger.debug("[CacheDependencyService] Cascade invalidated #{cache_key} and #{dependent_keys.size} dependents")
    end
    ...
    def selective_invalidate(cache_key)
      ...
      Rails.logger.debug("[CacheDependencyService] Selectively invalidated #{cache_key} and #{dependent_keys.size} direct dependents")
    end
    ...
    def update_instead_of_invalidate(cache_key)
      ...
      if cache_key.match?(/restaurant_dashboard:(\d+)/)
        ...
        locale = cache_key.match(/:([a-z]{2}):/) ? $1 : 'en'
        include_inactive = cache_key.include?(':true')
        fresh_data = AdvancedCacheService.cached_menu_with_items(menu_id, locale: locale, include_inactive: include_inactive)
    ...
    def expand_pattern_to_keys(pattern)
    ...
    def generate_cache_patterns_for_model(model, operation, changed_attributes)
      ...
      when 'Restaurant'
        ...
        if changed_attributes.include?('name') || operation == :destroy
          patterns << "user_activity:#{model.user_id}:*" if model.respond_to?(:user_id)
        end
      ...
      when 'Menuitem'
        if model.menusection && model.menusection.menu
    ...
    def update_derived_caches(cache_key, fresh_data)
      ...
      if cache_key.match?(/restaurant_dashboard:(\d+)/)
        ...
        update_restaurant_summary_caches($1.to_i, fresh_data)
      end
    end
    ...
    def warm_cache_key(cache_key, context, priority)
    ...
    def estimate_regeneration_time(cache_keys)
      ...
      cache_keys.each do |key|
        case key
        when /restaurant_dashboard/ then total_time += 500 # 500ms
        when /menu_full/ then total_time += 300 # 300ms
        when /order_analytics/ then total_time += 1000 # 1s
        when /menu_performance/ then total_time += 800 # 800ms
        else total_time += 100 # 100ms default
        end
      end
    ...
    def estimate_memory_impact(cache_keys)
      ...
      cache_keys.each do |key|
        case key
        when /restaurant_dashboard/ then total_memory += 50_000 # 50KB
        when /menu_full/ then total_memory += 100_000 # 100KB
        when /order_analytics/ then total_memory += 200_000 # 200KB
        when /menu_performance/ then total_memory += 75_000 # 75KB
        else total_memory += 10_000 # 10KB default
        end
      end
