...
module BeverageIntelligence
  class WhiskeyParser
    ...
    IB_NAMES = [
      'gordon & macphail', 'signatory', 'cadenhead', 'berry bros', 'douglas laing', 'hunter laing', 'adelphi', 'blackadder', 'murray mcdavid', 'compass box', 'wemyss', 'scotch malt whisky society', 'smws', 'that boutique-y', "chieftain's", 'duncan taylor',
    ].freeze
    ...
    def parse(menuitem)
      ...
      if (m = AGE_REGEX.match(t))
        ...
        if (m = BARE_AGE_REGEX.match(after_distillery))
          age = m[1].to_i
          fields['age_years'] = age if age.between?(3, 50)
        end
      end
