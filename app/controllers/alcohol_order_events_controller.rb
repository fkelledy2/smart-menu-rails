class AlcoholOrderEventsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.json { render json: events.as_json(only: [:id, :ordr_id, :ordritem_id, :menuitem_id, :employee_id, :customer_sessionid, :alcoholic, :abv, :alcohol_classification, :age_check_acknowledged, :acknowledged_at, :created_at]) }
    end
  end
