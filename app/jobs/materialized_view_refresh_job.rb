class MaterializedViewRefreshJob < ApplicationJob
  ...
  def refresh_specific_view(view_name, force_refresh)
  ...
  def refresh_by_priority(priority_level)
    ...
    if priority_level.to_sym == :high && success_count < total_count
      failed_views = results.select { |_, result| !result[:success] }.keys
  ...
  def refresh_stale_views
    ...
    Rails.logger.info "[MaterializedViewRefreshJob] Found #{stale_views.size} stale views: #{stale_views.map { |v| v[:view] }.join(', ')}"
    ...
    if failed_high_priority.any?
      failed_view_names = failed_high_priority.map { |r| r[:view] }
