...
module ResponsiveImageHelper
  ...
  def picture_tag_with_webp(image, alt:, sizes: '100vw', class_name: '', loading: 'lazy', quality: 85)
    ...
    content_tag(:picture) do
      ...
      if webp_src.present?
        sources << tag(:source, 
          srcset: webp_srcset.presence || webp_src,
          sizes: sizes,
          type: 'image/webp'
        )
      end
      ...
      sources << tag(:source,
        srcset: original_srcset.presence || original_src,
        sizes: sizes,
  ...
  def optimized_image_tag(image, alt:, class_name: '', quality: 85)
    ...
    src = optimized ? url_for(optimized) : url_for(blob)
  ...
  def image_dimensions(image)
    return { width: 0, height: 0 } unless image.present?

    blob = image.is_a?(ActiveStorage::Attachment) ? image.blob : image
    return { width: 0, height: 0 } unless blob.present?
