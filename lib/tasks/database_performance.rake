namespace :db do
  namespace :performance do
    ...
    task analyze: :environment do
      ...
      unused_indexes = ActiveRecord::Base.connection.execute(<<~SQL.squish)
      ...
      missing_fk_indexes = ActiveRecord::Base.connection.execute(<<~SQL.squish)
      ...
      index_stats = ActiveRecord::Base.connection.execute(<<~SQL.squish)
        SELECT schemaname, relname as tablename, indexrelname as indexname, idx_scan, idx_tup_read, idx_tup_fetch
        FROM pg_stat_user_indexes#{' '}
        WHERE idx_scan > 0
      ...
      table_sizes = ActiveRecord::Base.connection.execute(<<~SQL.squish)
    ...
    task check_long_queries: :environment do
      ...
      long_queries = ActiveRecord::Base.connection.execute(<<~SQL.squish)
        SELECT pid,#{' '}
