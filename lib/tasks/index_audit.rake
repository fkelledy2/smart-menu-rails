...
namespace :db do
  ...
  task index_audit: :environment do
    ...
    composite_suggestions = [
      { table: 'menus', columns: %w[restaurant_id status],
        reason: 'Common query: find active menus for restaurant', },
      { table: 'ordrs', columns: %w[restaurant_id status], reason: 'Common query: find open orders for restaurant' },
      { table: 'ordrs', columns: %w[tablesetting_id status], reason: 'Common query: find orders for table' },
      { table: 'menuitems', columns: %w[menusection_id sequence], reason: 'Common query: ordered items in section' },
      { table: 'menusections', columns: %w[menu_id sequence], reason: 'Common query: ordered sections in menu' },
    ]

    composite_suggestions.each do |suggestion|
      next unless ActiveRecord::Base.connection.table_exists?(suggestion[:table])
