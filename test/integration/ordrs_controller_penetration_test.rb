...
class OrdrsControllerPenetrationTest < ActionDispatch::IntegrationTest
  ...
  test 'cannot modify other customer orders' do
    ...
    @order3.total if @order3.respond_to?(:total)
  ...
  test 'payment processing requires proper authorization' do
    ...
    if respond_to?(:process_payment_restaurant_ordr_path)
      post process_payment_restaurant_ordr_path(@restaurant1,
                                                @order1,)
    end
    if response
  ...
  test 'cannot manipulate payment amounts' do
    ...
    original_total = if @order1.respond_to?(:total_amount)
                       @order1.total_amount
                     else
                       (@order1.respond_to?(:total) ? @order1.total : 0)
                     end
  ...
  test 'order status changes follow business rules' do
    ...
    valid_statuses = %w[opened confirmed preparing ready completed cancelled]
    ...
    if response.body.present?
      begin
        ...
        if json_response.is_a?(Array)
          order_ids = json_response.pluck('id')
  ...
  test 'order queries include proper authorization scoping' do
    ...
    subscription = ActiveSupport::Notifications.subscribe 'sql.active_record' do |_name, _start, _finish, _id, payload|
      queries << payload[:sql] if payload[:sql]&.include?('ordrs')
    end
