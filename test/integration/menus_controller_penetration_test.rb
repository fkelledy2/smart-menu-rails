...
class MenusControllerPenetrationTest < ActionDispatch::IntegrationTest
    ...
    if response.body.present?
      begin
        ...
        if json_response.is_a?(Array)
          menu_ids = json_response.pluck('id')
  ...
  test 'cannot bypass menu publishing workflow' do
    ...
    assert_includes %w[inactive active], draft_menu.status,
                    'Menu status should be handled according to business rules'
  end
  ...
  test 'menu status changes are properly authorized' do
    ...
    assert_includes %w[active archived], @menu1.status, 'Status should be handled according to business rules'
