...
class TablesettingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create tablesetting with different status values' do
    status_values = [:free, :occupied, :archived]
  ...
  test 'should handle capacity validation ranges' do
    ...
    valid_capacities.each_with_index do |capacity, index|
      post restaurant_tablesettings_url(@restaurant), params: {
        tablesetting: {
  ...
  test 'should handle tablesetting filtering by type' do
    ...
    Tablesetting.create!(name: 'Outdoor Table', capacity: 6, tabletype: :outdoor, status: :free, restaurant: @restaurant)
  ...
  test 'should handle tablesetting filtering by status' do
    ...
    Tablesetting.create!(name: 'Occupied Table', capacity: 4, tabletype: :indoor, status: :occupied, restaurant: @restaurant)
    Tablesetting.create!(name: 'Archived Table', capacity: 4, tabletype: :indoor, status: :archived, restaurant: @restaurant)
  ...
  test 'should handle large datasets efficiently' do
    ...
    20.times do |i|
      Tablesetting.create!(
        ...
        tabletype: [:indoor, :outdoor].sample,
        status: [:free, :occupied, :archived].sample,
        restaurant: @restaurant
      )
    end
