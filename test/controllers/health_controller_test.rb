...
class HealthControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should handle database health check failure gracefully' do
    ...
    ActiveRecord::Base.connection.stub(:execute, lambda { |*_args|
  ...
  test 'should return unhealthy status when database fails in full check' do
    ActiveRecord::Base.connection.stub(:execute, ->(*_args) { raise ActiveRecord::ConnectionNotEstablished }) do
      get health_full_path
      assert_response :service_unavailable
  ...
  test 'should handle cache stats error gracefully' do
    ...
    Rails.cache.stub(:respond_to?, lambda { |method|
      method == :redis ? true : Rails.cache.class.instance_method(:respond_to?).bind_call(Rails.cache, method)
    },) do
  ...
  test 'should handle redis read/write value mismatch' do
    ...
    Rails.cache.stub(:read, ->(_key) { 'different_value' }) do
      get health_redis_path
      assert_response :service_unavailable
  ...
  test 'should handle redis timeout scenarios' do
    Rails.cache.stub(:write, ->(*_args) { raise Timeout::Error, 'Operation timed out' }) do
      get health_redis_path
      assert_response :service_unavailable
  ...
  test 'should handle database connection pool exhaustion' do
    ActiveRecord::Base.connection.stub(:execute, lambda { |*_args|
      raise ActiveRecord::ConnectionTimeoutError, 'Pool exhausted'
    },) do
      get health_database_path
      assert_response :service_unavailable
  ...
  test 'should handle identity cache when disabled' do
    IdentityCache.stub(:respond_to?, ->(method) { method != :enabled? }) do
      get health_full_path
      assert_response :success
  ...
  test 'should handle identity cache fetch errors gracefully' do
    ...
    User.stub(:first, -> { user }) do
      User.stub(:fetch, ->(_id) { raise StandardError, 'Cache error' }) do
        get health_full_path
        ...
        assert json_response['services']['identity_cache']['status'].in?(%w[healthy degraded])
      end
    end
  end
  ...
  test 'should not expose sensitive information in error messages' do
    Rails.cache.stub(:write, ->(*_args) { raise StandardError, 'Database password: secret123' }) do
      get health_redis_path
      assert_response :service_unavailable
  ...
  test 'should handle mixed service states in full check' do
    ...
    ActiveRecord::Base.connection.stub(:execute, ->(*_args) { raise ActiveRecord::StatementInvalid }) do
      get health_full_path
      assert_response :service_unavailable
