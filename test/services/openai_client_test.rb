...
class OpenaiClientTest < ActiveSupport::TestCase
  ...
  test 'should edit image with mask' do
    ...
    @client.stub(:post, mock_post) do
      @client.edit_image(@test_image_path.to_s, edit_prompt, mask_path: mask_path)
  ...
  test 'should fetch available models' do
    mock_models_response = mock_openai_models_response([
      { 'id' => 'dall-e-2', 'object' => 'model', 'created' => 1_677_649_963 },
      { 'id' => 'dall-e-3', 'object' => 'model', 'created' => 1_698_785_189 },
    ])
  ...
  test 'should handle API errors in image generation' do
    mock_openai_error_response(400, 'Bad request')

    @client.stub(:post, ->(*_args) { raise ExternalApiClient::ApiError, 'API Error' }) do
  ...
  test 'should handle content policy violations' do
    @client.stub(:post, ->(*_args) { raise ExternalApiClient::ApiError, 'content policy violation' }) do
      error = assert_raises(OpenaiClient::InvalidPromptError) do
  ...
  test 'should handle billing/quota errors' do
    @client.stub(:post, ->(*_args) { raise ExternalApiClient::ApiError, 'billing limit exceeded' }) do
