...
class IntelligentCacheWarmingServiceTest < ActiveSupport::TestCase
  ...
  test 'should warm user context cache' do
    ...
    AdvancedCacheService.stub :cached_restaurant_dashboard, ->(id) { { restaurant_id: id } } do
      AdvancedCacheService.stub :cached_restaurant_orders, ->(_id, _opts) { { orders: [] } } do
        AdvancedCacheService.stub :cached_restaurant_employees, ->(_id, _opts) { { employees: [] } } do
          AdvancedCacheService.stub :cached_user_activity, ->(_id, _opts) { { activity: [] } } do
            AdvancedCacheService.stub :cached_user_all_orders, ->(_id) { { orders: [] } } do
              AdvancedCacheService.stub :cached_user_all_employees, ->(_id) { { employees: [] } } do
  ...
  test 'should warm restaurant context cache' do
    # Mock AdvancedCacheService methods
    AdvancedCacheService.stub :cached_restaurant_dashboard, ->(id) { { restaurant_id: id } } do
      AdvancedCacheService.stub :cached_order_analytics, ->(_id, _range) { { analytics: [] } } do
        AdvancedCacheService.stub :cached_restaurant_order_summary, ->(_id, _opts) { { summary: {} } } do
          AdvancedCacheService.stub :cached_restaurant_employee_summary, ->(_id, _opts) { { summary: {} } } do
            assert_nothing_raised do
  ...
  test 'should warm menu context cache' do
    # Mock AdvancedCacheService methods
    AdvancedCacheService.stub :cached_menu_with_items, ->(id, _opts) { { menu_id: id } } do
      AdvancedCacheService.stub :cached_menu_items_with_details, ->(_id, _opts) { { items: [] } } do
        AdvancedCacheService.stub :cached_menu_performance, ->(_id, _opts) { { performance: {} } } do
          AdvancedCacheService.stub :cached_section_items_with_details, ->(_id) { { section_items: [] } } do
            # Mock CacheKeyService
            CacheKeyService.stub :warm_menu_cache, ->(_menu) { true } do
              assert_nothing_raised do
        ...
        IntelligentCacheWarmingService.stub :warm_dinner_cache, -> { true } do
          IntelligentCacheWarmingService.stub :warm_night_cache, -> { true } do
            ...
            [7, 12, 18, 23].each do |hour|
              Time.stub :current, Time.zone.local(2025, 1, 1, hour, 0, 0) do
                assert_nothing_raised do
  ...
  test 'should warm cache for business events' do
    # Mock the individual warming methods to avoid database queries
    IntelligentCacheWarmingService.stub :warm_menu_context, ->(_id, _opts) { true } do
      IntelligentCacheWarmingService.stub :warm_restaurant_context, ->(_id, _opts) { true } do
        IntelligentCacheWarmingService.stub :warm_employee_cache, ->(_id, _opts) { true } do
          events = [
  ...
  test 'should perform scheduled cache warming' do
    ...
    IntelligentCacheWarmingService.stub :warm_restaurant_context, ->(_id, _opts) { true } do
      IntelligentCacheWarmingService.stub :warm_menu_context, ->(_id, _opts) { true } do
        ...
        mock_restaurant_chain.define_singleton_method(:where) { |_conditions| mock_restaurant_chain }
        mock_restaurant_chain.define_singleton_method(:group) { |_field| mock_restaurant_chain }
        mock_restaurant_chain.define_singleton_method(:order) { |_order| mock_restaurant_chain }
        mock_restaurant_chain.define_singleton_method(:limit) { |_limit| mock_restaurant_chain }
        mock_restaurant_chain.define_singleton_method(:find_each) do |&block|
        ...
        mock_menu_chain.define_singleton_method(:where) { |_conditions| mock_menu_chain }
        mock_menu_chain.define_singleton_method(:group) { |_field| mock_menu_chain }
        mock_menu_chain.define_singleton_method(:order) { |_order| mock_menu_chain }
        mock_menu_chain.define_singleton_method(:limit) { |_limit| mock_menu_chain }
        mock_menu_chain.define_singleton_method(:find_each) do |&block|
  ...
  test 'should handle morning cache warming' do
    Time.stub :current, Time.zone.local(2025, 1, 1, 8, 0, 0) do
  ...
  test 'should handle lunch cache warming' do
    Time.stub :current, Time.zone.local(2025, 1, 1, 12, 0, 0) do
  ...
  test 'should handle dinner cache warming' do
    Time.stub :current, Time.zone.local(2025, 1, 1, 19, 0, 0) do
  ...
  test 'should handle night cache warming' do
    Time.stub :current, Time.zone.local(2025, 1, 1, 2, 0, 0) do
  ...
  test 'should handle cache warming errors gracefully' do
    ...
    AdvancedCacheService.stub :cached_restaurant_dashboard, ->(_id) { raise StandardError, 'Cache error' } do
  ...
  test 'should respect cache tier settings' do
    ...
    %i[hot warm cold archive].each do |tier|
      assert_nothing_raised do
  ...
  test 'should integrate with CacheKeyService' do
    CacheKeyService.stub :warm_menu_cache, lambda { |menu|
  ...
  test 'should not cause memory leaks during warming' do
    ...
    IntelligentCacheWarmingService.stub :warm_user_context, ->(_id, _opts) { true } do
