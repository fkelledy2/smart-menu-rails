...
class CacheUpdateServiceTest < ActiveSupport::TestCase
  ...
  test 'should warm cache for new restaurant on create operation' do
    ...
    IntelligentCacheWarmingService.stub :warm_restaurant_context, -> (id, tier:) {
  ...
  test 'should invalidate cache for restaurant on destroy operation' do
    ...
    CacheDependencyService.stub :invalidate_for_model_change, -> (model, operation) {
  ...
  test 'should update menu cache on update operation' do
    # Mock AdvancedCacheService methods
    AdvancedCacheService.stub :cached_menu_with_items, -> (id, opts) { 
      { menu: { id: id, name: 'Updated Menu' } } 
    } do
      AdvancedCacheService.stub :cached_menu_items_with_details, -> (id, opts) { 
        { items: [] } 
      } do
        AdvancedCacheService.stub :cached_restaurant_dashboard, -> (id) { 
          { restaurant: { id: id } } 
  ...
  test 'should warm cache for new menu on create operation' do
    IntelligentCacheWarmingService.stub :warm_menu_context, -> (id, tier:) {
  ...
  test 'should update menu item in existing cache' do
    ...
    @restaurant.stub :restaurantlocales, -> {
      mock_relation = Object.new
      mock_relation.define_singleton_method(:pluck) { |field| ['en'] }
      mock_relation
    } do
  ...
  test 'should perform batch cache updates' do
    ...
    CacheUpdateService.stub :update_restaurant_cache, -> (obj, operation:) {
      restaurant_updated = true
    } do
      CacheUpdateService.stub :update_menu_cache, -> (obj, operation:) {
        menu_updated = true
      } do
  ...
  test 'should perform smart restaurant update for name changes' do
    changed_attributes = ['name', 'status']
  ...
  test 'should perform partial restaurant update for minor changes' do
    ...
    # Mock serialization
    CacheUpdateService.stub :serialize_restaurant_basic, -> (restaurant) {
  ...
  test 'should handle array delta updates' do
    ...
    CacheUpdateService.stub :apply_array_delta, -> (array, delta) {
      result = array.dup
