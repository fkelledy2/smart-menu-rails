...
class CacheWarmingServiceTest < ActiveSupport::TestCase
  ...
  test 'should handle errors gracefully in warm_all' do
    ...
    @service.stub(:warm_metrics, -> { raise StandardError, 'Test error' }) do
      log_output = capture_logs do
        result = @service.warm_all
  ...
  test 'should generate admin metrics summary' do
    ...
    QueryCacheService.stub(:warm_cache, lambda { |configs|
  ...
  test 'should generate restaurant order statistics' do
    ...
    mock_orders.define_singleton_method(:sum) { |_field| 500.0 }
    mock_orders.define_singleton_method(:average) { |_field| 50.0 }
    mock_orders.define_singleton_method(:where) { |_conditions| mock_orders }
  ...
  test 'should generate user restaurants data' do
    ...
    mock_restaurants.define_singleton_method(:includes) { |*_args| mock_restaurants }
    mock_restaurants.define_singleton_method(:limit) { |_n| mock_restaurants }
  ...
  test 'should handle database errors gracefully' do
    ...
    Restaurant.stub(:joins, ->(*_args) { raise ActiveRecord::ConnectionNotEstablished, 'Database error' }) do
  ...
  test 'should handle QueryCacheService errors gracefully' do
    ...
    QueryCacheService.stub(:warm_cache, -> { raise StandardError, 'Cache service error' }) do
