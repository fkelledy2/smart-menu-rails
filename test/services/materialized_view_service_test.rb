...
class MaterializedViewServiceTest < ActiveSupport::TestCase
  ...
  test "should have correct materialized views configuration" do
    ...
    MaterializedViewService::MATERIALIZED_VIEWS.each do |view_name, config|
      ...
      assert [:high, :medium, :low].include?(config[:priority]), "#{view_name} priority should be high, medium, or low"
    end
  end
  ...
  test "should identify views supporting concurrent refresh" do
    ...
    refute @service.send(:supports_concurrent_refresh?, 'system_analytics_mv')
    refute @service.send(:supports_concurrent_refresh?, 'dw_orders_mv')
  end
  ...
  test "should perform health check" do
    ...
    assert [:healthy, :degraded, :unhealthy].include?(health_check[:overall_status])
