...
class CacheDependencyServiceTest < ActiveSupport::TestCase
  ...
  test 'should perform cascade invalidation' do
    ...
    call_count = 0
    CacheDependencyService.stub :find_dependent_keys, -> (key) { 
      call_count += 1
  ...
  test 'should perform selective invalidation' do
    ...
    # Mock dependent keys
    CacheDependencyService.stub :find_dependent_keys, -> (key) { ["menu:#{@menu.id}:full"] } do
      Rails.cache.write("menu:#{@menu.id}:full", { menu: 'data' })
  ...
  test 'should update cache instead of invalidating' do
    ...
    AdvancedCacheService.stub :cached_restaurant_dashboard, -> (id) { 
  ...
  test 'should perform batch invalidation' do
    ...
    CacheDependencyService.stub :find_dependent_keys, -> (key) { [] } do
  ...
  test 'should invalidate cache for restaurant model changes' do
    ...
    CacheDependencyService.stub :invalidate_pattern, -> (pattern) { 
  ...
  test 'should invalidate cache patterns' do
    ...
    Rails.cache.stub :delete_matched, -> (p) { 
  ...
  test 'should fallback to pipeline service for pattern invalidation' do
    ...
    Rails.stub :cache, cache_without_delete_matched do
      RedisPipelineService.stub :bulk_invalidate_patterns, -> (patterns) {
  ...
  test 'should analyze invalidation impact' do
    ...
    CacheDependencyService.stub :find_all_dependent_keys, -> (key, visited) { 
      ["menu:#{@menu.id}:full", "order:1:analytics"] 
    } do
      CacheDependencyService.stub :estimate_regeneration_time, -> (keys) { 1500 } do
        CacheDependencyService.stub :estimate_memory_impact, -> (keys) { 150000 } do
          ...
          assert_equal 150000, impact[:memory_impact]
  ...
  test 'should warm dependent caches' do
    ...
    CacheDependencyService.stub :find_dependent_keys, -> (key) { 
      ...
      CacheDependencyService.stub :warm_cache_key, -> (key, ctx, priority) {
  ...
  test 'should handle invalidation errors gracefully' do
    ...
    Rails.cache.stub :delete, -> (key) { raise Redis::ConnectionError, 'Connection failed' } do
