...
class RedisPipelineServiceTest < ActiveSupport::TestCase
  ...
  test 'should perform bulk cache write' do
    ...
    @test_data.each do |key, expected_value|
      ...
      assert_equal expected_value, cached_value,
                   "Expected #{key} to have value #{expected_value}, but got #{cached_value}"
    end
  end
  ...
  test 'should perform bulk cache read' do
    ...
    assert_equal @test_data.size, results.size,
                 "Expected #{@test_data.size} results, got #{results.size}. Results: #{results.inspect}"
    @test_data.each do |key, expected_value|
      assert_equal expected_value, results[key],
                   "Expected #{key} to have value #{expected_value}, but got #{results[key]}"
    end
  end
  ...
  test 'should perform bulk cache delete' do
    ...
    RedisPipelineService.bulk_cache_delete(@test_data.keys)
  ...
  test 'should check bulk cache existence' do
    ...
    keys = %w[existing_key non_existing_key]
  ...
  test 'should invalidate cache patterns' do
    ...
    if has_working_redis
      ...
      assert deleted_count.positive?, 'Expected some keys to be deleted'
    else
  ...
  test 'should handle mixed data types in bulk operations' do
    ...
    mixed_data.each do |key, expected_value|
      assert_equal expected_value, results[key],
                   "Failed for key: #{key}. Expected: #{expected_value.inspect}, Got: #{results[key].inspect}"
    end

    # Clean up
    mixed_data.each_key { |key| Rails.cache.delete(key) }
  end
end
