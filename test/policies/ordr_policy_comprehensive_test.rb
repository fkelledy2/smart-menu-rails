...
class OrdrPolicyComprehensiveTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @tablesetting = Tablesetting.create!(restaurant: @restaurant, name: 'Table 1', tabletype: :indoor, capacity: 4,
                                         status: :free,)
    @other_tablesetting = Tablesetting.create!(restaurant: @other_restaurant, name: 'Table 1', tabletype: :indoor,
  ...
  test 'should prevent cross-restaurant order access' do
    ...
    assert_not result, "User should not be able to edit other user's Ordr"
  end
  ...
  test 'should handle different order statuses' do
    statuses = %i[opened ordered billrequested paid closed]
  ...
  test 'should handle order feature authorization' do
    features = %i[analytics performance bulk_update]
