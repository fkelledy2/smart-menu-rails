...
class RestaurantOnboardingJobTest < ActiveJob::TestCase
  ...
  test 'should track analytics events' do
    ...
    AnalyticsService.stub :track_onboarding_completed, lambda { |user, data|
      analytics_calls << [:onboarding_completed, user, data]
    } do
      AnalyticsService.stub :track_restaurant_created, lambda { |user, restaurant|
        analytics_calls << [:restaurant_created, user, restaurant]
      } do
        AnalyticsService.stub :track_menu_created, ->(user, menu) { analytics_calls << [:menu_created, user, menu] } do
          AnalyticsService.stub :identify_user, lambda { |user, traits|
            analytics_calls << [:identify_user, user, traits]
          } do
            RestaurantOnboardingJob.perform_now(@user.id, @onboarding.id)
          end
        end
  ...
  test 'should call SmartMenuGeneratorJob perform_async' do
    ...
    SmartMenuGeneratorJob.stub :perform_async, lambda { |restaurant_id|
