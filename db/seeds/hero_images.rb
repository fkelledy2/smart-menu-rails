...
Rails.logger.debug 'Seeding hero images...'
...
hero_images_data.each do |image_data|
  ...
  if hero_image.new_record?
    ...
    if hero_image.save
      Rails.logger.debug { "  ✓ Created hero image: #{image_data[:alt_text]} (sequence: #{image_data[:sequence]})" }
    else
      Rails.logger.debug { "  ✗ Failed to create hero image: #{hero_image.errors.full_messages.join(', ')}" }
    end
  else
    Rails.logger.debug { "  - Hero image already exists: #{image_data[:alt_text]}" }
  end
end

Rails.logger.debug 'Hero images seeding complete!'
Rails.logger.debug { "Total hero images: #{HeroImage.count}" }
Rails.logger.debug { "Approved hero images: #{HeroImage.approved.count}" }
