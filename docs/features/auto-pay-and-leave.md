...
## Customer Flows
- Add Payment Method
  1. Tap “Add Payment” on SmartMenu.
  2. Hosted payment element (PSP) collects card, returns a token/payment_method_id.
  3. Backend stores provider + reference; sets `payment_on_file=true`, `payment_on_file_at=now`.
  4. Staff view shows “Payment on File”.

- View Bill
  1. Tap “View Bill” to see live totals, taxes, tip options.
  2. Backend sets `viewed_bill_at=now` on first view.

- Enable Auto-Pay
  1. Toggle “Auto-Pay when ready”.
  2. Confirm consent dialog explains when the charge will occur.
  3. Backend sets `auto_pay_enabled=true`, `auto_pay_consent_at=now`.
