...
## Status
- Priority Rank: #22 (Phase 2 agent — most complex UX integration; ship after back-office agent patterns are proven by Growth Digest and Optimization agents)
- Category: Post-Launch — agent tier, Phase 2
- Effort: L
- Dependencies: Agent Framework (#15), back-office controllers and service objects (existing), `RestaurantInsightsService` (existing), `ProfitMarginAnalyticsService` (existing), OpenAI API with streaming
