...
## Status
- Priority Rank: #18 (Phase 1 agent — high differentiation value; ship after Framework + at least one back-office agent proves the toolbox)
- Category: Post-Launch — agent tier, Phase 1
- Effort: M
- Dependencies: Agent Framework (#15), SmartMenu customer view (existing), `Allergyn` model (existing), `Menuparticipant` model (existing), OpenAI API with streaming
